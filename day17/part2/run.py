        ...
        while z < len(cube_array[0]):
            ...
            while y < len(cube_array[0]):
                ...
                while x < len(cube_array[0]):
                    active_neighbor_count = get_active_neighbors(cube_array, x, y, z, w)

                    #if active but doesn't have 2 or 3 active neighbors, flip
                    if (cube_array[w][z][y][x] == 1 and 
                        (active_neighbor_count != 2 and
                        active_neighbor_count != 3)):
                        temp_array[w][z][y][x] = 0
                    
                    #if inactive and has 3 active neighbors, flip
                    elif (cube_array[w][z][y][x] == 0 and 
                        active_neighbor_count == 3):
                        temp_array[w][z][y][x] = 1
                    
                    else:
                        temp_array[w][z][y][x] = cube_array[w][z][y][x]
...
def get_active_neighbors(cube_array, x, y, z, w):
    active_count = 0
    ...
    print('Actives for {0}, {1}, {2}, {3}'.format(w, z, y, x))

    while current_w <= min(len(cube_array[0])-1, w+1):
        current_z = max(0, z-1)
        while current_z <= min(len(cube_array[0])-1, z+1):
            ...
            while current_y <= min(len(cube_array[0])-1, y+1):
                ...
                while current_x <= min(len(cube_array[0])-1, x+1):
                    if (current_x != x or current_y != y or current_z != z or current_w != w):
                        if cube_array[current_w][current_z][current_y][current_x] == 1:

                            #print('{0}, {1}, {2}, {3} - 1'.format(current_w, current_z, current_y, current_x))
                            active_count+=1
                        #else:
                            #print('{0}, {1}, {2}, {3} - 0'.format(current_w, current_z, current_y, current_x))
...
def set_starting_positions(num_cycles, lines, max_bounds):
    cube_array = np.zeros((max_bounds,max_bounds,max_bounds,max_bounds)) # 4D array

    y = 0
    z = int(max_bounds/2) - int(len(lines)/2)
    w = int(max_bounds/2) - int(len(lines)/2)
    ...
    for line in lines:
        ...
        for cube in list(line):
            ...
            cube_array[w][z][y+num_cycles][x+num_cycles] = active_val
            x+=1
        y+=1
